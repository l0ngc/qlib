...
class LSTM(Model):
    ...
    def train_epoch(self, x_train, y_train, weight):
        x_train_values = x_train.values
        y_train_values = np.squeeze(y_train.values)
        weight_values = np.squeeze(weight.values)
        ...
        for i in range(len(indices))[:: self.batch_size]:
            ...
            tmp_weight = torch.from_numpy(weight_values[indices[i : i + self.batch_size]]).float().to(self.device)

            pred = self.lstm_model(feature)
            loss = self.loss_fn(pred, label, tmp_weight)
    ...
    def test_epoch(self, data_x, data_y, weight):
    ...
    def fit(
        ...
        if reweighter is None:
            wl_train = np.ones(len(x_train))
            wl_valid = np.ones(len(x_valid))
        elif isinstance(reweighter, Reweighter):
            wl_train = reweighter.reweight(x_train)
            wl_valid = reweighter.reweight(x_valid)
        ...
        for step in range(self.n_epochs):
            ...
            train_loss, train_score = self.test_epoch(x_train, y_train, wl_train)
            val_loss, val_score = self.test_epoch(x_valid, y_valid, wl_valid)

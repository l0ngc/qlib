...
BASE_ADDR = '/16t-2/cl/project/qlib/examples/benchmarks_dynamic/baseline'
BENCH_ADDR = '/16t-2/cl/project/qlib/examples/benchmarks'
output_dir = '/16t-2/cl/project/qlib/examples/us_day_res/RR'

# models = os.listdir('/16t-2/cl/project/qlib/examples/benchmarks')
# tgt_models = ['Linear', 'LightGBM', 'Transformer', 'XGBoost', 'MLP']
...
for model in tgt_models:
    try:
        ...
        # conf_name = f'workflow_config_{model.lower()}_Alpha360.yaml'
        conf_addr = f'{model_dir}/{conf_name}'

        print(conf_addr)
        print(f'python rolling_benchmark_us.py --conf_path={conf_addr} run')
        # # # Running shell commands
        subprocess.run(f'rm -rf mlruns', shell=True)
        subprocess.run(f'python rolling_benchmark_us.py --conf_path={conf_addr} run', shell=True)
        # subprocess.run(f'python rolling_benchmark.py --conf_path={conf_addr} run', shell=True)
